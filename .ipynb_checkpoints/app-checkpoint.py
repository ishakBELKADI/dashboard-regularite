...
from data_loader import get_realtime_df, compute_regularite_metrics, save_regularite_history, load_regularite_history
                        ...
                        html.Div(
                            ...
                            children=[
                                ...
                                html.Div(
                                    ...
                                    children=[dcc.Graph(id="terminus-chart")],
                                ),
                            ],
                        ),
                    ],
...
def update_dashboard(n, choix_regularite, choix_regularite_terminus):
...
# =========================
    ...
    nb_dessertes_actuelles = metrics["nb_dessertes"]
    nb_regulieres_actuelles = metrics["nb_dessertes_regulieres"]

    nb_dessertes_projetees = len(df)

    nb_dessertes_restantes = max(
        nb_dessertes_projetees - nb_dessertes_actuelles,
        0
    )

    # Cas idéal : toutes les dessertes restantes sont régulières
    nb_dessertes_regulieres_projetees = (
        nb_regulieres_actuelles + nb_dessertes_restantes
    )

    taux_accostage_desserte = (
        round(nb_dessertes_regulieres_projetees / nb_dessertes_projetees * 100, 2)
        if nb_dessertes_projetees > 0
        else 0
    )

    # Scénario : 10 dessertes restantes non régulières
    nb_dessertes_irregulieres_scenario_10 = min(10, nb_dessertes_restantes)

    nb_dessertes_regulieres_scenario_10 = (
        nb_regulieres_actuelles
        + max(nb_dessertes_restantes - nb_dessertes_irregulieres_scenario_10, 0)
    )

    taux_accostage_desserte_scenario_10 = (
        round(nb_dessertes_regulieres_scenario_10 / nb_dessertes_projetees * 100, 2)
        if nb_dessertes_projetees > 0
        else 0
    )

    # Scénario : 20 dessertes restantes non régulières
    nb_dessertes_irregulieres_scenario_20 = min(20, nb_dessertes_restantes)

    nb_dessertes_regulieres_scenario_20 = (
        nb_regulieres_actuelles
        + max(nb_dessertes_restantes - nb_dessertes_irregulieres_scenario_20, 0)
    )

    taux_accostage_desserte_scenario_20 = (
        round(nb_dessertes_regulieres_scenario_20 / nb_dessertes_projetees * 100, 2)
        if nb_dessertes_projetees > 0
        else 0
    )
    # Scénario : 50 dessertes restantes non régulières
    nb_dessertes_irregulieres_scenario_50 = min(50, nb_dessertes_restantes)

    nb_dessertes_regulieres_scenario_50 = (
        nb_regulieres_actuelles
        + max(nb_dessertes_restantes - nb_dessertes_irregulieres_scenario_50, 0)
    )

    taux_accostage_desserte_scenario_50 = (
        round(nb_dessertes_regulieres_scenario_50 / nb_dessertes_projetees * 100, 2)
        if nb_dessertes_projetees > 0
        else 0
    )
    ...
    df_gare["arrivee_reguliere"] = df_gare["arrival_delay_s"] <= SEUIL_REGULARITE_S
    ...
    per_train = (
        ...
        .agg(
            ...
            nb_regulieres=("desserte_reguliere", "sum"),
        )
    )
    ...
    if not per_train.empty:
        per_train["taux_regularite"] = (
            per_train["nb_regulieres"] / per_train["nb_dessertes"] * 100
        ).round(2)
    ...
    if not df_finished.empty:
        ...
        terminus_df["terminus_regulier"] = terminus_df["retard_s"] <= SEUIL_REGULARITE_S
        terminus_df["etat"] = terminus_df["terminus_regulier"].map({
            True: "Régulier",
            False: "Irrégulier"
        })

        fig_terminus = px.histogram(
            terminus_df,
            x="etat",
            title="Bilan régularité au terminus"
        )
        ...
        fig_terminus = px.histogram(title="Aucun train terminé")
...
def toggle_popup_trains(open_clicks, close_clicks, tab_value, retard_subtab, search_value, irreguliers_subtab):    
    ...
    df_scope["desserte_reguliere_calc"] = (
        df_scope["desserte_prise_en_compte"] &
        (df_scope["retard_s"] <= SEUIL_REGULARITE_S)
    )
    ...
    summary = (
        ...
        .agg(
            ...
            nb_dessertes_irregulieres=("retard_s", lambda x: int((x > SEUIL_REGULARITE_S).sum())),
            retard_max_s=("retard_s", "max"),
            retard_max_m=("retard_m", "max"),
